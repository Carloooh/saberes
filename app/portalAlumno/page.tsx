...
const diasJustificados = asistencia.filter(
  (dia) => dia.estado === "Justificado"
).length;
const diasAusentes = asistencia.filter(
  (dia) => dia.estado === "Ausente"
).length;
const porcentajeAsistencia = (
  ((diasPresentes + diasJustificados) / totalDias) *
  100
).toFixed(1);
...
const calificacionesConPromedio = calificaciones.map((cal) => ({
  ...cal,
  promedio: Number(
    (cal.notas.reduce((a, b) => a + b, 0) / cal.notas.length).toFixed(2)
  ),
}));

const promedioTotal = Number(
  (
    calificacionesConPromedio.reduce((sum, cal) => sum + cal.promedio, 0) /
    calificacionesConPromedio.length
  ).toFixed(2)
);
              ...
              <tbody>
                {calificacionesConPromedio.map((calificacion) => (
                  <tr key={calificacion.asignatura}>
                    ...
                    <td className="p-2 border-t font-semibold">
                      {calificacion.promedio}
                    </td>
                  </tr>
                ))}
                <tr className="bg-gray-50">
                  <td className="p-2 border-t font-semibold" colSpan={4}>
                    Promedio Total
                  </td>
                  <td className="p-2 border-t font-semibold">
                    {promedioTotal}
