...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get the requested RUT from the URL if it exists
    const url = new URL(request.url);
    const requestedRut = url.searchParams.get("rut");

    // If a specific RUT is requested, verify permissions
    if (requestedRut && requestedRut !== userSession.rut_usuario) {
    ...
    // Use either the requested RUT or the session user's RUT
    const rutUsuario = requestedRut || userSession.rut_usuario;
