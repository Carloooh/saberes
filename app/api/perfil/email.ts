...
export const sendWelcomeEmailTeacher = async (
  email: string,
  name: string,
  rut: string,
  password: string
  ...
  const message = {
    ...
    to: email,
    subject:
      "Bienvenido a Saberes El Quisco - Credenciales de acceso para Docentes",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        <h2 style="color: #333; text-align: center;">Bienvenido a Saberes El Quisco</h2>
        <p>Hola ${name},</p>
        ...
        <p>Como docente, tendrás acceso a las funcionalidades específicas para gestionar tus cursos y estudiantes asignados.</p>
  ...
  try {
    await transporter.sendMail(message);
    console.log(`Correo de bienvenida para docente enviado a ${email}`);
    return true;
  } catch (error) {
    console.error(
      "Error al enviar el correo de bienvenida para docente:",
      error
    );
    throw error;
  }
