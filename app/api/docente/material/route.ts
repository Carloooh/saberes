...
export async function GET(request: Request) {
  try {
    ...
    // Verificar si la asignatura pertenece al curso especificado
    const checkStmt = db.prepare(`
      SELECT * FROM Asignatura
      WHERE id_asignatura = ? AND id_curso = ?
    `);
    const asignatura = checkStmt.get(asignaturaId, cursoId);

    if (!asignatura) {
      return NextResponse.json(
        { success: false, error: "La asignatura no pertenece al curso especificado" },
        { status: 404 }
      );
    }
