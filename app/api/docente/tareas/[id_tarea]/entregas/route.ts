  ...
  return new Promise<NextResponse>((resolve, reject) => {
    connection.on("connect", async (err) => {
      ...
      try {
        ...
        const id_asignatura =
          searchParams.get("id_asignatura") || searchParams.get("asignatura");
        const id_tarea =
          searchParams.get("id_tarea") || request.url.split("/").slice(-2)[0];
        const id_curso =
          searchParams.get("id_curso") || searchParams.get("curso");
        ...
        console.log(
          `Processing request for tarea: ${id_tarea}, asignatura: ${id_asignatura}, curso: ${id_curso}`
        );
