...
const AccionesGenerales = () => {
  ...
  const handleEnviarBienvenida = async () => {
    if (!confirm("¿Estás seguro de enviar mensajes de bienvenida a todos los estudiantes activos? Esto generará nuevas contraseñas para todos ellos.")) {
      return;
    }

    setLoading("bienvenida");
    try {
      const response = await fetch("/api/administrador/acciones-generales/bienvenida", {
        method: "POST",
      });
      ...
      if (data.success) {
        toast.success(`Se enviaron ${data.enviados} mensajes de bienvenida correctamente.`);
      } else {
  ...
  return (
    <div className="space-y-6 p-6 bg-white rounded-lg shadow">
      ...
      <p className="text-gray-600">
        Estas acciones afectan a múltiples usuarios a la vez. Úsalas con precaución.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Tarjeta de Bienvenida */}
        <div className="border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
          <h3 className="text-xl font-semibold text-gray-800">Bienvenida a los alumnos</h3>
          <p className="text-gray-600 mt-2 mb-4">
            Envío de mensaje de bienvenida a todos los alumnos y sus credenciales de inicio de sesión.
          </p>
          <button
            ...
            className={`border border-blue-600 text-blue-600 bg-white px-4 py-2 rounded-md transition-colors
              ${loading === "bienvenida" ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-600 hover:text-white"}`}
          >
        ...
        <div className="border border-gray-200 rounded-lg p-6 shadow-sm bg-gray-50">
          <h3 className="text-xl font-semibold text-gray-800">Cambiar de curso</h3>
          <p className="text-gray-600 mt-2 mb-4">
            Pasar de curso a todos los alumnos que cumplan con los criterios mínimos.
          </p>
          <button
            disabled={true}
