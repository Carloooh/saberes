...
import ModalSeleccionUsuarios from "./ModalSeleccionUsuarios";
...
const AccionesGenerales = () => {
  ...
  const handleEnviarEmailSeleccionados = async (
    ...
    try {
      const response = await fetch(
        "/api/administrador/acciones-generales/enviar-email-seleccionados",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div>
      <div className="space-y-6 p-6 bg-white rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Tarjeta de Email a Usuarios Seleccionados */}
          <div className="border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
            <h3 className="text-xl font-semibold text-gray-800">
            ...
            <button
              ...
              disabled={loading === "emailSeleccionados"}
              className={`border border-blue-600 text-blue-600 bg-white px-4 py-2 rounded-md transition-colors
                ${
                  loading === "emailSeleccionados"
                    ? "opacity-50 cursor-not-allowed"
                    : "hover:bg-blue-600 hover:text-white"
                }`}
            >
      ...
      <ModalSeleccionUsuarios
        ...
        loading={loading === "emailSeleccionados"}
      />
    </div>
  );
};
